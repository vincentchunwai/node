...
class USBAdapter extends adapter.Adapter {
  constructor(vid, pid) {
    super();
    this.device = null;
    this.endpoint = null;
    ...
    } else if (vid && vid instanceof usb.usb.Device) {
      this.device = vid;
    } else {
    ...
    usb.usb.on("detach", this.detachDevice);
  }
  detachDevice(device) {
  ...
  static findPrinter() {
    return usb.getDeviceList().filter((device) => USBAdapter.isPrinter(device));
  }
  ...
  static getDeviceBySerial(serialNumber) {
    return new Promise(async (resolve, reject) => {
      try {
        const device = await usb.findBySerialNumber(serialNumber);
        device?.open();
        resolve(device);
      } catch (err) {
        reject(err);
  ...
  close(callback) {
    ...
    try {
      this.device?.close();
      usb.usb.removeListener("detach", this.detachDevice);
      callback && callback(null);
